...
static rgbdata_t *CustomDecal_LoadHLWADSpray( const void *raw, int size )
{
	...
	if( memcmp( hdr->identification, "WAD3", 4 ) != 0 )
		return NULL;
	...
	{
		if( lumps[i].type == WAD3_SPRAY_TYPE || lumps[i].type == WAD3_MIPTEX_TYPE )
		{
			mipdata = (const unsigned char *)raw + lumps[i].filepos;

			if( lumps[i].disksize < sizeof( miptex_t ) + 2 + 768 )
				continue;
			...
			if( lumps[i].type == WAD3_SPRAY_TYPE )
			{
				// gradient palette
			...
			for( j = 0; j < (int)(width * height); ++j )
			{
				...
				if( alpha_mode == 1 )
				{
					out->buffer[i * 4 + 3] = idx; // soft transparency (0x40)
				}
				else
				{
					out->buffer[i * 4 + 3] = ( idx == 255 ) ? 0 : 255; // classic (0x43)
				}
			}
			return out;
		}
